...
object NotchHelper {
    ...
    fun has3rdNotch(context: Context?): Boolean {
        context?.run {
            ...
            } else if (ScreenUtil.isVivo()) {
                return hasNotchInVivo(this)
            } else if (ScreenUtil.isOppo()) {
                return hasNotchInOppo(this)
            } else if (ScreenUtil.isXiaomi()) {
    ...
    fun hasNotchInXiaomi(context: Context?): Boolean {
        try {
            ...
            val getMethod = spClass.getDeclaredMethod("getInt", String::class.java,
    ...
    private fun get3rdSafeInsetRect(context: Context?): Rect? { // 全面屏设置项更改
        if (ScreenUtil.isHuawei()) {
            val isHuaweiNotchSetToShow: Boolean = ScreenUtil.huaweiIsNotchSetToShowInSetting(context)
    ...
    private fun getRectInfoRotation0(context: Context?): Rect? {
        val rect = Rect()
        if (ScreenUtil.isVivo()) { // TODO vivo 显示与亮度-第三方应用显示比例
            rect.top = getNotchHeightInVivo(context)
            rect.bottom = 0
        } else if (ScreenUtil.isOppo()) { // TODO OPPO 设置-显示-应用全屏显示-凹形区域显示控制
    ...
    fun needFixLandscapeNotchAreaFitSystemWindow(view: View?): Boolean {
        return (ScreenUtil.isXiaomi() || ScreenUtil.isVivo()) && hasNotch(view) ?: false
    }
}